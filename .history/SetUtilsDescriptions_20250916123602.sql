...
EXEC util.metadataSetFunctionDescription 
    ...
    @description = N'Універсальна функція для отримання ідентифікатора будь-якого об''єкта бази даних за його іменем та класом об''єкта.

Повертає:
INT - ідентифікатор об''єкта або NULL якщо об''єкт не знайдено

Приклади:
SELECT util.metadataGetAnyId(''dbo.Users'', ''1'') -- отримати ID таблиці
SELECT util.metadataGetAnyId(''dbo.Users'', ''1'', ''UserName'') -- отримати ID стовпця';

EXEC util.metadataSetFunctionDescription 
    @function = 'util.metadataGetColumnId',
    @description = N'Отримує ідентифікатор стовпця таблиці за ідентифікатором таблиці та іменем стовпця.

Повертає:
INT - column_id стовпця з sys.columns або NULL якщо стовпець не знайдено

Приклад:
SELECT util.metadataGetColumnId(OBJECT_ID(''dbo.Users''), ''UserName'')';

EXEC util.metadataSetFunctionDescription 
    @function = 'util.metadataGetIndexId',
    @description = N'Отримує ідентифікатор індексу за іменем таблиці та іменем індексу.

Повертає:
INT - index_id індексу з sys.indexes або NULL якщо індекс не знайдено

Приклад:
SELECT util.metadataGetIndexId(''dbo.Users'', ''IX_Users_Email'')';

EXEC util.metadataSetFunctionDescription 
    @function = 'util.metadataGetParameterId',
    @description = N'Отримує ідентифікатор параметра процедури або функції за іменем об''єкта та іменем параметра.

Повертає:
INT - parameter_id параметра з sys.parameters або NULL якщо параметр не знайдено

Приклад:
SELECT util.metadataGetParameterId(''dbo.GetUserById'', ''@UserId'')';

EXEC util.metadataSetFunctionDescription 
    @function = 'util.metadataGetDataspaceId',
    @description = N'Отримує ідентифікатор простору даних (dataspace) за його іменем.

Повертає:
INT - data_space_id з sys.data_spaces або NULL якщо простір даних не знайдено
...
EXEC util.metadataSetFunctionDescription 
    ...
    @description = N'Отримує ідентифікатор функції розділення (partition function) за її іменем.

Повертає:
INT - function_id з sys.partition_functions або NULL якщо функція не знайдена

Приклад:
SELECT util.metadataGetPartitionFunctionId(''pf_DateRange'')';
...
Повертає:
NVARCHAR(128) - ім''я об''єкта або NULL якщо об''єкт не знайдено

Приклад:
SELECT util.metadataGetAnyName(12345, ''1'') -- отримати ім''я таблиці за ID';
...
Повертає:
NVARCHAR(128) - повне ім''я об''єкта у форматі [schema].[object] або NULL якщо об''єкт не знайдено

Приклад:
SELECT util.metadataGetObjectName(OBJECT_ID(''dbo.Users''))';
...
Повертає:
NVARCHAR(128) - ім''я стовпця або NULL якщо стовпець не знайдено

Приклад:
SELECT util.metadataGetColumnName(OBJECT_ID(''dbo.Users''), 1)';
...
Повертає:
NVARCHAR(128) - ім''я індексу або NULL якщо індекс не знайдено

Приклад:
SELECT util.metadataGetIndexName(OBJECT_ID(''dbo.Users''), 2)';
...
Повертає:
NVARCHAR(128) - ім''я параметра включаючи @ або NULL якщо параметр не знайдено

Приклад:
SELECT util.metadataGetParameterName(OBJECT_ID(''dbo.GetUserById''), 1)';
...
Повертає:
NVARCHAR(128) - ім''я сертифіката або NULL якщо сертифікат не знайдено
...
Повертає:
NVARCHAR(128) - ім''я файлової групи або NULL якщо простір даних не знайдено
...
Повертає:
NVARCHAR(128) - ім''я функції розділення або NULL якщо функція не знайдена

Приклад:
SELECT util.metadataGetPartitionFunctionName(65536)';
...
Повертає:
INT - код класу або NULL для невідомого класу
Коди класів: 0-База даних, 1-Об''єкт/стовпець, 2-Індекс, 3-Схема, 4-Користувач, 7-Тип, 20-Простір даних, 21-Функція розділення, 22-Файл, 25-Сертифікат

Приклад:
SELECT util.metadataGetClassByName(''OBJECT_OR_COLUMN'') -- поверне 1';
...
Повертає:
NVARCHAR(128) - назва класу об''єкта або NULL для невідомого коду

Приклад:
SELECT util.metadataGetClassName(1) -- поверне ''OBJECT_OR_COLUMN''';
...
Повертає:
NVARCHAR(60) - опис типу об''єкта або NULL якщо об''єкт не знайдено
Приклади типів: ''USER_TABLE'', ''SQL_STORED_PROCEDURE'', ''SQL_SCALAR_FUNCTION''

Приклад:
SELECT util.metadataGetObjectType(OBJECT_ID(''dbo.Users''))';
...
Повертає таблицю:
type CHAR(2) - код типу об''єкта
type_desc NVARCHAR(60) - опис типу об''єкта

Приклад:
SELECT * FROM util.metadataGetObjectsType()';
...
Повертає таблицю:
objtype NVARCHAR(128) - тип об''єкта
objname NVARCHAR(261) - ім''я об''єкта
name NVARCHAR(128) - назва властивості
value SQL_VARIANT - значення властивості

Приклад:
SELECT * FROM util.metadataGetDescriptions()';
...
Приклад:
SELECT * FROM util.metadataGetExtendedProperiesValues(''dbo.Users'', ''1'')';
