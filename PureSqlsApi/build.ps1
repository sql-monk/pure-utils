...
<#
.SYNOPSIS
    Build script for PureSqlsApi project
.DESCRIPTION
...
$ProjectFile = Join-Path $SourcePath "PureSqlsApi.csproj"

Write-Host "=====================================" -ForegroundColor Cyan
Write-Host "  PureSqlsApi Build Script" -ForegroundColor Cyan
Write-Host "=====================================" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    if ($Clean) {
        ...
        Get-Process -Name "PureSqlsApi" -ErrorAction SilentlyContinue | Stop-Process -Force
    ...
    if (Test-Path $publishPath) {
        ...
        Write-Host "PureSqlsApi.exe" -ForegroundColor Green
        ...
        Write-Host "  .\bin\$Configuration\net8.0\$Runtime\publish\PureSqlsApi.exe --server localhost --database TestDB --port 5000" -ForegroundColor Gray
        Write-Host "  .\bin\$Configuration\net8.0\$Runtime\publish\PureSqlsApi.exe -s myserver -d mydb -u user -p 8080" -ForegroundColor Gray
        Write-Host ""
    }
    else {
