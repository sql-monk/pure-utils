...
- **Microsoft SQL Server**: версія 2016 або новіша
  - Рекомендується SQL Server 2019 або новіша для повної підтримки всіх функцій
  - Необхідні права `db_owner` або `sysadmin` для встановлення
...
### Помилка прав доступу при розгортанні

Переконайтеся, що ваш користувач має права `db_owner` або `sysadmin` на сервері SQL Server.

### Extended Events сесії не запускаються

Перевірте, чи існує шлях для файлів XE:

```sql
-- Отримати шлях за замовчуванням
SELECT * FROM util.xeGetLogsPath();
```

Якщо виникли інші проблеми, перегляньте [FAQ](faq.md) або створіть issue на GitHub.
