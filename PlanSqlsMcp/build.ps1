...
<#
.SYNOPSIS
    Build script for PlanSqlsMcp project
.DESCRIPTION
...
$ProjectFile = Join-Path $SourcePath "PlanSqlsMcp.csproj"

Write-Host "=====================================" -ForegroundColor Cyan
Write-Host "  PlanSqlsMcp Build Script" -ForegroundColor Cyan
Write-Host "=====================================" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    if ($Clean) {
        ...
        Get-Process -Name "PlanSqlsMcp" -ErrorAction SilentlyContinue | Stop-Process -Force
    ...
    Write-Host "  Trimmed: Yes (link-level)" -ForegroundColor Gray
    Write-Host ""
    ...
    # Show results
    $publishPath = ".\bin\$Configuration\net8.0\$Runtime\publish\PlanSqlsMcp.exe"
    ...
    if (Test-Path $publishPath) {
        ...
        Write-Host "PlanSqlsMcp.exe" -ForegroundColor Green
        ...
        Write-Host "  .\bin\$Configuration\net8.0\$Runtime\publish\PlanSqlsMcp.exe --server localhost --database master" -ForegroundColor Gray
        Write-Host "  .\bin\$Configuration\net8.0\$Runtime\publish\PlanSqlsMcp.exe -s myserver -d mydb -u user -p password" -ForegroundColor Gray
        Write-Host "  .\bin\$Configuration\net8.0\$Runtime\publish\PlanSqlsMcp.exe --help" -ForegroundColor Gray
        Write-Host ""
    }
    else {
