...
### 🌐 pureAPI - HTTP адаптер для SQL Server API
Python мікросервіс, що надає HTTP доступ до SQL-об'єктів у схемі `api`:

- **[pureAPI](pureAPI/)** - тонкий HTTP-адаптер на базі FastAPI:
  - Автоматична маршрутизація до SQL функцій та процедур
  - Підтримка табличних функцій (`/{resource}/list`), скалярних функцій (`/{resource}/get`), процедур (`/exec/{name}`)
  - Використання pymssql для підключення до SQL Server
  - Windows та SQL Server автентифікація
  - Мінімальна конфігурація через CLI параметри
  - [Детальна документація](pureAPI/README.md)
...
### 🔗 Схема api - об'єкти для HTTP доступу

SQL об'єкти для використання через pureAPI мікросервіс:
- [`databasesList`](api/Functions/databasesList.sql), [`databaseGet`](api/Functions/databaseGet.sql) - приклади функцій для HTTP endpoints
- [`testOperation`](api/Procedures/testOperation.sql) - приклад процедури з OUTPUT параметром для HTTP виклику
- Всі об'єкти повертають JSON для автоматичної серіалізації через HTTP
